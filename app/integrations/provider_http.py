...
def request_with_retries(
    ...
    *,
    ...
    Важно:
    В httpx 0.28.x параметр stream нельзя передавать напрямую в client.request().
    Если нужен потоковый ответ, запрос создаётся через build_request(), а затем
    отправляется через client.send(..., stream=True).
    """
    ...
    back = (
        settings.provider_http_retry_backoff_base
    ...
    with httpx.Client(http2=False, follow_redirects=True) as client:
        for attempt in range(max_r + 1):
            try:
                ...
                if stream:
                    req = client.build_request(method_u, url, **request_kwargs)
                ...
                log.error(
                    "[%s] %s %s окончательно не удалось: %s",
                    label,
                    method_u,
                    safe_url,
                    exc,
